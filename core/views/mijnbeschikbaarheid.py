...
def mijnbeschikbaarheid_view(request):
    ...
    # NL-weergave forceren voor deze pagina
    translation.activate("nl")

    today = timezone.localdate()
    WEEKS_AHEAD = 26  # 6 maanden vooruit
    ...
    week_end = monday + timedelta(days=4)  # ✅ juiste einddatum
    ...
    # Rijen voor de template (lost checkbox-issue)
    rows = []
    for d in days:
        av = existing.get(d)
        rows.append({
    ...
    context = {
        "monday": monday,
        "week_end": week_end,            # ✅ voor correcte range in template
        "rows": rows,
    ...
    return render(request, "mijnbeschikbaarheid/index.html", context)