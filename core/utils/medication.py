...
def group_meds_by_jansen(geneesmiddelen_lijst, overrides_lookup=None):
    ...
    Verplichte lege groepen:
      -1 = Contra-indicaties en allergieën
       0 = Labwaarden
       1 = Vallen?
       2 = Malen?
    ...
    groepen = {
        -1: {"naam": jansen_map.get(-1, "Contra-indicaties en allergieën"), "meds": []},
    ...
    for gm in geneesmiddelen_lijst:
        ...
        # Placeholder groups never contain medication rows.
        if gid in (-1, 0, 1, 2):
            continue

        if override_gid is not None:
            group_name = jansen_map.get(gid, "Overig")
        else:
            group_name = (gm.get("ATC3_jansen_naam") or jansen_map.get(gid) or "Overig")

        if gid not in groepen:
            groepen[gid] = {"naam": group_name, "meds": []}
        elif (groepen[gid].get("naam") in (None, "", "Overig")) and group_name:
            groepen[gid]["naam"] = group_name
