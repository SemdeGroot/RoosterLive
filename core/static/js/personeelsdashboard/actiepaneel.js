(() => {
  ...
  function shiftCardHeader(item){
    ...
    else if (item.kind === "editing") pillKind = "editing";
    ...
    return `
      <div class="pd-shift-card">
        <div class="pd-shift-pill-wrap">
          ${pill}
        </div>
        <div class="pd-shift-text">
  ...
  function selectionCard(item) {
    ...
    const isEditing =
      state.dirty.has(k) ||
      (!item.existing && (a.location_id || a.task_id));

    const headerKind = isEditing ? "editing" : item.kind;
    ...
    card.innerHTML = `
      <div class="pd-item-main">
        ${shiftCardHeader({ ...item, kind: headerKind })}
      </div>
  ...
  function panelShiftCard(shiftLike) {
    ...
    const kind = shiftLike.kind || "available";
    ...
    taskSel.addEventListener("change", () => {
      const cur = state.assigned.get(k) || { location_id: String(locSel.value || ""), task_id: "" };
  ...
  function shouldHideBaseSlot(k) {
    if (!state.selected.has(k)) return false;
    if (!state.dirty.has(k)) return false;

    const a = state.assigned.get(k);
    if (!a) return false;

    return !!(a.location_id || a.task_id);
  }
  ...
  function renderPanels() {
    ...
    effective.forEach(s => {
      ...
      const k = keyFromParts(s.user_id, s.date, s.period);
      if (shouldHideBaseSlot(k)) return;
  ...
  if (publishBtn) {
    publishBtn.addEventListener("click", async (e) => {
      ...
      const msg =
        ...
        "Dit publiceert alle wijzigingen van deze week.";
...
function hasUnsavedChanges() {
  ...
  for (const it of state.selected.values()) {
    ...
    const k = keyOf(it);
    const a = state.assigned.get(k);
    if (a && (a.location_id || a.task_id)) return true;
  }
  return false;
}

function beforeUnloadHandler(e) {
  if (_suppressBeforeUnload) return;
...
async function maybeSaveThenNavigate(navigateFn) {
  ...
  const ok = confirm(
    ...
    "Wil je dit opslaan als concept?\n\n" +
    "OK = opslaan en doorgaan\n" +
    "Annuleren = niet opslaan en doorgaan"
  );
...
function bindUnsavedGuards() {
  ...
  if (weekMenu) {
    weekMenu.addEventListener('click', (e) => {
      ...
      const target = opt.dataset.value;
      if (!target) return;
