document.addEventListener("DOMContentLoaded", function() {
    ...
    // ==========================================
// 5. COPY FOR MEDIMO (per Jansen categorie)
// ==========================================

function buildMedimoTextFromGroup(groupEl, groupId) {
    // 1) Medicatie + gebruik uit de tabel
    ...
    // 2) Opmerkingen / historie uit de textarea(s)
...
document.querySelectorAll(".btn-copy-medimo").forEach((btn) => {
    btn.addEventListener("click", async () => {
        ...
        const text = buildMedimoTextFromGroup(groupEl, groupId);
        ...
        try {
            ...
            // Kleine UX feedback
            const original = btn.querySelector(".copy-label")?.innerText || "Kopieer voor Medimo";
            if (btn.querySelector(".copy-label")) btn.querySelector(".copy-label").innerText = "Gekopieerd!";
            btn.classList.add("is-copied");

            setTimeout(() => {
                if (btn.querySelector(".copy-label")) btn.querySelector(".copy-label").innerText = original;
                btn.classList.remove("is-copied");
            }, 1200);
        } catch (e) {
            console.error("Kopiëren mislukt:", e);
            if (btn.querySelector(".copy-label")) btn.querySelector(".copy-label").innerText = "Kopiëren mislukt";
            setTimeout(() => {
                if (btn.querySelector(".copy-label")) btn.querySelector(".copy-label").innerText = "Kopieer voor Medimo";
            }, 1200);
        }
    });
});
